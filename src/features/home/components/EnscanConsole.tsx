...
export default function EnscanConsole() {
  ...
  const AddTargetForm = () => (
    <div className="space-y-4">
      ...
      <textarea
        placeholder="描述 (可选)"
        className="w-full bg-surface-dim border border-outline-variant rounded-lg p-3 text-on-surface focus:ring-2 focus:ring-primary outline-none h-24 resize-none"
      />
  ...
  const ResultsView = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {[
        { label: '子域名发现', value: '42', color: 'primary' },
        { label: '开放端口', value: '15', color: 'secondary' },
        { label: '安全风险', value: '3', color: 'tertiary' },
      ].map((stat) => (
        <div key={stat.label} className="bg-surface-container-low rounded-xl p-6 border border-outline-variant/30 text-center">
          <p className="text-label-caps text-outline mb-2">{stat.label}</p>
          <p className={`text-headline-xl font-headline-xl text-${stat.color}`}>{stat.value}</p>
        </div>
      ))}
      <div className="md:col-span-3 space-y-4">
        <h4 className="text-label-caps text-outline">高风险资产清单</h4>
        <div className="space-y-2">
          {[
            { name: 'admin.example.com', risk: '弱口令风险', color: 'tertiary' },
            { name: 'dev-db.example.com', risk: '数据库公开访问', color: 'error' },
          ].map((item) => (
            <div key={item.name} className="flex items-center justify-between p-3 bg-surface-dim rounded border-l-4 border-${item.color}">
              <span className="font-data-mono text-sm text-on-surface">{item.name}</span>
              <span className={`text-[10px] bg-${item.color}/10 text-${item.color} px-2 py-1 rounded`}>{item.risk}</span>
            </div>
          ))}
        </div>
      </div>
